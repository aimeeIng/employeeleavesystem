...
const getAllEmployees = async (req, res) => {
    ...
    if (!employees) return res.status(204).json({ 'message': 'No available employees.' });
    res.json(employees);
}
const createEmployee = async (req, res) => {
    if (!req?.body?.firstname || !req?.body?.lastname || !req?.body?.department) {
        return res.status(400).json({ 'message': 'Firstname, Lastname and Department are required' });
    }
    try {
